#!/usr/bin/env python3
"""Validate JSONL trades against integration/trade_schema.json.

This is a tiny schema-first guardrail for CI/smoke. It is intentionally
dependency-free (no jsonschema package).
...
def load_schema(schema_path: str) -> Dict[str, Any]:
    ...
    if schema.get("schema_version") != "trade_v1":
        raise ValueError("schema_version must be trade_v1")
...
def validate_record(obj: Dict[str, Any], schema: Dict[str, Any], lineno: int) -> List[str]:
    ...
    if "schema_version" in obj and obj.get("schema_version") != "trade_v1":
...
def main() -> int:
        ...
        with open(args.jsonl, "r", encoding="utf-8") as f:
            for lineno, raw in enumerate(f, start=1):
                ...
                if not isinstance(obj, dict):
                    ...
                    continue
                parsed += 1
                errs = validate_record(obj, schema, lineno)
    ...
    if args.format == "json":
        ...
        if ok:
            print(f"[validate_trade_jsonl_json] OK total_lines={total} parsed_lines={parsed}")
        else:
            for e in all_errs[: args.max_errors]:
                _eprint(e)
            _eprint(f"[validate_trade_jsonl_json] FAIL errors={len(all_errs)} total_lines={total}")
